 ...
 * Register different handler to speed up.
 */
...
static atomic64_t	gtp_sim_clock_count;

static u64
gtp_sim_clock(void)
{
	return atomic64_inc_return(&gtp_sim_clock_count);
}

#ifndef GTP_SELF_RING_BUFFER
struct gtp_ring_buffer {
	unsigned			pages;
	unsigned			flags;
	int				cpus;
...
static void gtp_ring_buffer_set_clock(struct gtp_ring_buffer *buffer,
				      u64 (*clock)(void))
{
	buffer->clock = clock;
}
...
gtp_gdbrsp_qtinit(void)
{
	...
	gtp_have_pc_pe = 0;
#endif
	atomic64_set(&gtp_sim_clock_count, 0);
...
restart:
	...
	if (!gtp_frame) {
		...
		ring_buffer_set_clock(gtp_frame, gtp_sim_clock);
#else
		gtp_ring_buffer_set_clock((struct gtp_ring_buffer *)gtp_frame,
					  gtp_sim_clock);
#endif
#endif
		if (!gtp_frame)
